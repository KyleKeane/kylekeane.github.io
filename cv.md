---
layout: default
title: CV
permalink: /cv.html
description: Full record — roles, publications, grants and awards, press, and skills.
nav_include: true
nav_order: 8
---

## Overview

The full archival record. The other pages are curated; this one is
comprehensive. For current research, see [Research](/research.html); for
forthcoming engagements, [Upcoming](/upcoming.html); for engagement
categories, [Engage](/engage.html).

## Roles

### Senior Lecturer in Assistive Technologies

University of Bristol — School of Computer Science, June 2024 – present.
Teaching and research focusing on multisensory interaction, human–computer
interaction, immersive audio, agentic AI, human echolocation and
localisation, fingertip haptics, and perception-aware robotics. Continuing
affiliation as Researcher at the
[MIT Spatial Sound Lab](https://spatialsoundlab.mit.edu/), co-leading the
*Accessible Technology and Disability Justice* research theme with
Nelly Kate Anderson — see the
[Spatial Sound Lab Research page](https://spatialsoundlab.mit.edu/research/)
for the theme description and the ongoing Kara/Keane sonification
collaboration.

### Lecturer

Massachusetts Institute of Technology — Electrical Engineering and Computer
Science, September 2022 – May 2024. Lead faculty for *Principles and
Practices of Assistive Technology* (cross-listed as 6.450 / 6.811 / 2.78 /
HST.420), the capstone engineering subject pairing student teams with a
community member to co-invent technology that improves their independence
on a specific task. Co-instructor of *6.3900: Introduction to Machine
Learning*. Departmental
[MIT EECS people page](https://www.eecs.mit.edu/people/kyle-keane/).

### AI Research Scientist

MIT — College of Computing, Quest for Intelligence, September 2019 –
September 2022. AI research consultant and researcher across multiple labs,
building capacity for interdisciplinary research across human learning,
brain and cognitive science, AI, and societal modelling. Projects:
automated metadata tagging of unstructured humanities and social-science
datasets; AI vs human performance on out-of-distribution facial
recognition; AI-enhanced tactification of computer graphics; an
open-source visual-stimulus repository for human-vs-machine vision
comparison; statistical forecasting of infectious-disease spread in urban
universities.

### Lecturer (Materials Science)

MIT — Materials Science and Engineering, June 2015 – September 2019.
Created and led *3.008: Humanistic Co-design of Assistive Technology in the
Developing World*, a three-week subject taking students to India to work
with local people with disabilities. Co-taught undergraduate subjects
including *Computational Methods for Materials Science and Engineering*,
*Principles and Practices of Assistive Technology*, and *Making for
Innovation, Science, and Outreach*. Computational Curriculum Advisor on the
Undergraduate Committee. Staff sponsor and advisor for MIT's Assistive
Technology Hackathon (ATHack). 2018 MIT Institute Award (James N. Murphy)
and Infinite Mile Award. Faculty / fellowship-team page on the
[CoCreate Fellowship](https://cocreate20.org/team/kyle-keane/).

### Research Scientist (Interactive Materials Education Laboratory)

MIT — Materials Science and Engineering, June 2015 – September 2019.
Founder and director of an undergraduate research lab using technology to
increase public engagement, data literacy, and science education through
designed interactive experiences.
[IMEL @ MIT homepage](https://imel.mit.edu/).

### Technical Consultant

Wolfram Research — Communications and Strategy, October 2015 – September
2019. Educational outreach programmes including hackathon sponsorships and
mentorship; accessibility-related technical advisement and disability
advocacy training; rapid prototyping of new technologies and applications.

### Research Scientist (Solid-State Dewetting)

MIT — Carl V. Thompson Research Group, Materials Science and Engineering,
January 2016 – January 2017. Led the development of a large-scale
atomistic kinetic Monte Carlo simulation for solid-state dewetting in thin
metal films, GPU-parallelised using Bayesian statistics.

### Head of User Experience

Indico Data Solutions, March 2015 – October 2015. Managed website design
and usability, community engagement, developer relations, and the
documentation of a RESTful machine-learning API.

### Research Programmer — Educational Software

Wolfram Research — Educational Technology and Software, January 2014 –
March 2015. Designed and shipped the first software system for automated
step-by-step solution of calculations including physical units —
deployed to millions of students through the
[Wolfram\|Alpha step-by-step solver](https://www.wolframalpha.com/examples/pro-features/step-by-step-solutions).
Built a customisable curriculum-development platform for educators
teaching computer programming to children. My Wolfram-side authored
posts and discussions live on the
[Wolfram Blog author archive](https://blog.wolfram.com/author/kyle-keane/)
and the
[Wolfram Community member profile](https://community.wolfram.com/web/kylek);
2018 conference talks are listed on the
[Wolfram Technology Conference 2018 presentations page](https://www.wolfram.com/events/technology-conference/2018/presentations/).

### Research Programmer — Special Projects

Wolfram Research — Special Projects, November 2012 – December 2013.
Designed and programmed accessibility features for blind users of
Wolfram\|Alpha, CDF Player, and Mathematica. Wrote grants for
disability-access research, including qualitative and quantitative
data-collection design.

### R&D Fellow

Wolfram Research — Special Projects, July 2012 – October 2012.
Comprehensive review of the company's products for usability by people
with disabilities. Prepared Section 508 Voluntary Product Accessibility
Templates for all major products. Created a functioning workaround for
completely blind users to access Mathematica's computational capabilities.

### Graduate Student Researcher

University of California, Riverside — Department of Electrical
Engineering, with Dr. Alexander Korotkov, January 2009 – August 2012.
Weak quantum measurements, uncollapsing, quantum error correction and
detection, decoherence suppression, and quantum state and process
tomography for quantum computation and communication. Procedures
proposed in this work were subsequently verified in the originally
proposed superconducting-qubit system and others.

### Teaching Assistant / Laboratory Instructor

UC Riverside — Physics and Astronomy, September 2007 – December 2008
and March 2010 – June 2010. Calculus-based classical mechanics lab,
algebra-based and calculus-based electricity and magnetism lab, and
calculus-based modern physics discussion.

### Undergraduate Researcher

California State University, Fullerton — Physics and Astronomy,
August 2005 – August 2007. High-precision differential cross-section
measurements for electron impact on various targets; designed and
machined a rotary table for variable-geometry scattering experiments.

### Physics Tutor

CSUF — Physics and Astronomy, September 2005 – June 2007. Volunteer
tutor in the department's informal tutoring lounge.

## Publications

Across peer-reviewed journals, conference proceedings, and educational
resources, my work has been cited over 480 times (Google Scholar, late
2025). My ORCID identifier
[`0000-0003-3243-4412`](https://orcid.org/0000-0003-3243-4412) is the
canonical record. The Bristol-side publication and collaboration record
auto-updates on the
[University of Bristol Pure portal](https://research-information.bris.ac.uk/en/persons/kyle-m-keane/).

### Recent

- **A. Thompson and Kyle Keane**, *Architecting Perceptible Space — A
  Perceptually Validated Spatial Audio System for Echolocation Training.*
  Poster, Audio Developer Conference (ADC25), 10–12 November 2025,
  Bristol.
- **Shlomit Ben-Ami, Batel Buaron, Ori Yaron, Kyle Keane, Virginia H.
  Sun, Flip Phillips, Jason Friedman, Pawan Sinha, and Roy Mukamel,**
  *What the visual system can learn from the non-dominant hand: the
  effect of graphomotor engagement on visual discrimination.* *Memory
  & Cognition* **53**(1), 325–340 (2025).
  [Memory & Cognition article (DOI 10.3758/s13421-024-01628-2)](https://doi.org/10.3758/s13421-024-01628-2) ·
  [Open-access preprint on PsychArchives](https://www.psycharchives.org/en/item/429f2a4e-aa8f-49cc-8b0e-0147a2ef1d90).
- **Tegan Joy Roberts-Morgan, Brooke Morris, Elaine Czech, Suhan Neema,
  Abigale Stangl, Kyle Michael Keane, Matthew Horton, Janet Read, and
  Oussama Metatla,** *Sense-O-Nary — Exploring children's crossmodal
  metaphors through playful crossmodal interactions.* *IDC '24:
  Proceedings of the 23rd Annual ACM Interaction Design and Children
  Conference* (2024).
  [ACM Digital Library entry (DOI 10.1145/3628516.3655785)](https://doi.org/10.1145/3628516.3655785).
- **Catherine Holloway, Dafne Zuleima Morgado Ramirez, Tigmanshu
  Bhatnagar, Ben Oldfrey, Priya Morjaria, Soikat Ghosh Moulic, Ikenna D.
  Ebuenyi, Giulia Barbareschi, Fiona Meeks, Jessica Massie, Felipe
  Ramos-Barajas, Joanne McVeigh, Kyle Keane, George Torrens, P.V.M. Rao,
  Malcolm MacLachlan, Victoria Austin, Rainer Kattel, Cheryl D. Metcalf,
  Srinivasan Sujatha,** *A review of innovation strategies and processes
  to improve access to AT: Looking ahead to open innovation ecosystems.*
  *Assistive Technology* **33**(sup1), 68–86.

### Educational resources

- **Kyle Keane, Andrew Ringler, Abhinav Gandhi, and Mark Vrablic,**
  *Collaborative Design and Creative Expression using Arduino
  Microcontrollers.* MIT OpenCourseWare.
  [MIT OpenCourseWare resource for Collaborative Design and Creative Expression with Arduino](https://ocw.mit.edu/resources/res-3-002-collaborative-design-and-creative-expression-with-arduino-microcontrollers-january-iap-2017/).
- **Kyle Keane, Andrew Ringler, Abhinav Gandhi, and Mark Vrablic,**
  *Build Your Own Video Game with the Unity Game Engine and Microsoft
  Kinect.* MIT OpenCourseWare.
  [MIT OpenCourseWare resource for Learn to Build Your Own Videogame](https://ocw.mit.edu/resources/res-3-003-learn-to-build-your-own-videogame-with-the-unity-game-engine-and-microsoft-kinect-january-iap-2017/).

### Reports and standards work

- **Kyle Keane and Christina Laverentz,** *Interactive Scientific
  Graphics: Recommended Practices for Verbal Description.* DIAGRAM
  Center, 2014. Synthesis of guidelines for verbally communicating
  information conveyed in modern and interactive interfaces for
  scientific and quantitative content; informed the creation of the
  W3C User-Intent Working Group and was adopted by the PhET
  Interactive Simulations team at the University of Colorado.
  [Interactive Scientific Graphics report (DOCX)](http://diagramcenter.org/wp-content/uploads/2014/06/Interactive_Scientific_Graphics_Submission23june2014.docx).

### Edited and contributed chapters

- **Areej Al-Wabil, Shiroq Al-Megren, Kyle Keane, et al.,**
  *Empowering Assistive Technology Communities to Make Strategic
  Use of Intellectual Property: Three Case Studies from the
  CoCreate Program.* Springer, 2020.
  [Springer chapter (DOI 10.1007/978-3-030-60149-2_2)](https://link.springer.com/chapter/10.1007/978-3-030-60149-2_2)
  — peer-reviewed academic documentation of the Saudi-side
  CoCreate Fellowship case studies.

### Doctoral dissertation

- **Kyle Keane,** *Quantum State Protection and Transfer Using
  Superconducting Qubits.* University of California, Riverside, 2012.
  [Doctoral dissertation on eScholarship](http://escholarship.org/uc/item/8nq7q2hn).

### Peer-reviewed (physics)

- **Kyle Keane and Alexander Korotkov,** *Simplified quantum error
  detection and correction for superconducting qubits.* Phys. Rev. A
  **86**, 012333 (2012).
  [Phys. Rev. A volume 86 article 012333](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.86.012333).
- **Alexander N. Korotkov and Kyle Keane,** *Decoherence suppression by
  quantum measurement reversal.* Phys. Rev. A **81**, 040103(R) (2010).
  [Phys. Rev. A volume 81 rapid communication 040103](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.81.040103).
- **B. R. Yates, K. Keane, and M. A. Khakoo,** *Near-threshold electron
  impact doubly differential cross sections for the ionization of neon
  and xenon.* J. Phys. B: At. Mol. Opt. Phys. **42**, 095206 (2009).
  [J. Phys. B volume 42 article 095206 on IOPscience](http://iopscience.iop.org/article/10.1088/0953-4075/42/9/095206/meta).
- **M. A. Khakoo, J. Blumer, K. Keane, C. Campbell, H. Silva, M. C. A.
  Lopes, C. Winstead, V. McKoy, R. F. da Costa, L. G. Ferreira, M. A. P.
  Lima, and M. H. F. Bettega,** *Low-energy electron scattering from
  methanol and ethanol.* Phys. Rev. A **77**, 042705 (2008).
  [Phys. Rev. A volume 77 article 042705](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.77.042705).
- **M. A. Khakoo, K. Keane, C. Campbell, N. Guzman, and K. Hazlett,**
  *Low energy elastic electron scattering from ethylene.* J. Phys. B:
  At. Mol. Opt. Phys. **40**, 3601 (2007).
  [J. Phys. B volume 40 article 3601 on IOPscience](http://iopscience.iop.org/article/10.1088/0953-4075/40/18/003/meta).

## Grants as PI / co-PI

### 2022

- *Niche Advice Sharing Social Network for Individuals with Disabilities*
  — Johns Hopkins University, Disability Health Research Center.
- *Intercultural Education Exchange* — Al-Faisal University. Funding for
  research visits, workshops, and a 10-person MIT mentor cohort
  supporting an accessibility track in a 10-day game-a-thon.
- *Intercultural Education Exchange* — United States Embassy of Uruguay.
  Faculty training in collaborative design and assistive-technology
  co-design with local community members.
- *MIT–Brazil Discretionary Seed Fund.* Student travel to a humanitarian
  engineering trip to Uruguay.
- *MIT Undergraduate Research Opportunities Program (UROP).* Multiple
  funded projects.

### 2021

- *TranscribeGlass: An affordable heads-up augmented-reality real-time
  captioning device for the Deaf* — India–United States Technology
  Transfer Fund. Continuing advisor; project records on
  [MIT Solve](https://solve.mit.edu/challenges/re-engage-learners/solutions/62192)
  and the
  [companion MIT Solve solution page](https://solve.mit.edu/solutions/62010).
- *MIT UROP.*

### 2020

- *CoCreATe Riyadh* — Al-Faisal University. Funding from Saudi sources
  via Al-Faisal for an MIT cohort fellowship launch and remote
  mentorship of 20 engineering teams working with local individuals
  living with disabilities.
- *Online Primer for Co-Designing Assistive Technologies* — MITx
  Educational Innovation Fund.
- *Low-cost Vision Support Aids* — MIT Alumni Class Funds. Manufacturing
  research for low-cost vision aids in India.
- *TactionTablet: An Affordable Tactile Graphics Display for the Blind*
  — MIT Jameel World Education Laboratory. Year-long MEng thesis on a
  moving-actuator/passive-locking tactile pin display.
- *MIT UROP.*

### 2019

- *Assistive Technology in the Developing World* — MIT Alumni Class
  Funds. Month-long for-credit India course on service engineering.
- *Effectiveness of Infusing Computer Programming into Online
  Introductory Physics Classes* — MIT Integrated Learning Initiative
  (MITili). 2×2 factorial design across coding, interactive
  visualisations, math derivations, and conceptual reflection.
- *MIT UROP.*

### 2018

- *MIT Global Partnerships Seed Fund.* India for-credit course.
- *MIT–India Discretionary Fund.* India for-credit course.
- *Online Production of Creative Collaboration using Arduino* — MIT
  OpenCourseWare.
- *Collaborative Design using Arduino* — SUTD–MIT Educational Innovation
  and Exchange Fund. Two-week 60-person course pairing Singapore
  University of Technology and Design students with MIT students.
- *Online Production of Creative Collaboration using Unity Game Engine*
  — MIT OpenCourseWare.
- *Collaborative Design using Unity Game Engine* — SUTD–MIT Educational
  Innovation and Exchange Fund.
- *WolframLTI: Creating a Bi-directional LTI link for Wolfram Cloud and
  Mathematica Online to edX* — MITx Educational Innovation Fund.
- *MIT UROP.*

### 2017

- *Computational Storytelling to Bolster Learning* — MIT Jameel World
  Education Laboratory.
- *Adding Coding into General Institute Requirements* — MIT d'Arbeloff
  Education Fund.
- *Making the Freshman Year* — Dean's Action Group for Evidence-Based
  Learning.
- *MIT–India Discretionary Fund.* Seed funding for India partner
  development.
- *MIT UROP.*

### 2016

- *Making the Freshman Year* — Dean's Action Group for Evidence-Based
  Learning.
- *MIT UROP.*

### 2015

- *MIT UROP.*

### 2014

- *DIAGRAM Center for Educational Access.* Funding to research and
  synthesise best practices for creating educationally equivalent
  verbal descriptions of interactive scientific graphics.

## Grant support as researcher

- *Solid-State Dewetting of Metallic Thin Films* — NSF, 2015.
  Computational researcher.
- *Multi-qubit algorithms in Josephson phase qubits* — ARO/IARPA,
  2010–2015. Funded the doctoral dissertation.

## Awards

- **MIT Institute Award — James N. Murphy Award** (2018).
- **MIT School of Engineering Infinite Mile Award** (2018).

## Scholarships

- Molly Adams Scholarship — UC Riverside (2009).
- Chancellor's Distinguished Fellowship — UC Riverside (2007).
- Norman Nitzburg Award in Experimental Physics — CSU Fullerton (2006,
  2007).
- Wolfram Award in Computational Science — CSU Fullerton (2007).
- Outstanding Scholarship Award — CSU Fullerton (2007).
- Dan Black Scholarship — CSU Fullerton (2006, 2007).
- Kellogg Scholar — California Polytechnic University, Pomona (2002).
- California Governor's Scholarshare Award — Ruben S. Ayala High
  School (2002).

## Press and media

### 2025

- [How AI tools are transforming the lives of people with disabilities — WBUR On Point](https://www.wbur.org/onpoint/2025/07/09/ai-tools-disabilities-artificial-intelligence).
- [Scaffolding the Fantastical — Simon Mellins Consulting podcast, episode 15](https://www.simonmellins.com/podcast/episode15)
  ([video on YouTube](https://www.youtube.com/watch?v=B1soiP2DsHc)).
- [Macro Hive Conversations with Bilal Hafeez podcast featuring Kyle Keane](https://podcasts.apple.com/gb/podcast/macro-hive-conversations-with-bilal-hafeez/id1502904619).

### 2024

- [Sustainable Innovation of Assistive Technologies — Pervasive Media Studio at the Watershed (talk recording on YouTube)](https://www.youtube.com/watch?v=Bel1Hp6cuu0).
- [*Sights, sounds, sensations: Immersive art exhibition reSOUND at Culture Station Seoul 284* — d'strict press / Korea JoongAng Daily](https://www.dstrict.com/press/?bmode=view&idx=125557371).
- [reSOUND: Echo, Beyond Immersive Exhibition at Seoul 284 — NOL World](https://world.nol.com/en/content/festas/01904500-83ec-7237-bde0-610e3965864f).
- [Black Hole Reverb (Leftroman / Ian Condry, with my and Erin Kara's sonifications) on Immersive Audio Album](https://immersiveaudioalbum.com/product/black-hole-reverb-ian-condry-5-1-flac-dolby-atmos-mkv-mp4/).

### 2023

- [Listening to What Cannot Be Seen: MIT's Spatial Sound Lab Hosts an Open House — The Harvard Crimson](https://www.thecrimson.com/article/2023/5/30/mit-spatial-sound-lab-open-house-mitnano-immersion-lab/), 30 May 2023.

### 2022

- *Scientists tune into new ways of perceiving black holes* — *The New
  York Times*, 7 May 2022.
- [Astronomers are making maps using black hole echoes — BBC Sky at Night Magazine](https://www.skyatnightmagazine.com/news/making-maps-black-hole-echoes).
- [Astronomy and Sonification — Perkins School for the Blind](https://www.perkins.org/resource/astronomy-and-sonification/).
- [Intersensory data representations of a black hole — MIT Schwarzman College of Computing](https://sqi.mit.edu/news/intersensory-data-representations-black-hole).
- [Watch the black-hole sonification on YouTube](https://www.youtube.com/watch?v=iIeIag2Ji8k).
- [Intersensory data representations of black hole — MIT Quest for Intelligence](https://quest.mit.edu/news/intersensory-data-representations-black-hole).
- [Humanistic design workshops in India: impact and reach — MIT MISTI](https://misti.mit.edu/humanistic-design-workshops-india-impact-and-reach).

### 2021

- [Undergraduates explore practical applications of artificial intelligence — MIT News](https://news.mit.edu/2021/undergraduates-explore-practical-applications-artificial-intelligence-0503).
- [Search reveals eight new sources of black hole echoes — MIT News](https://news.mit.edu/2022/search-reveals-eight-new-sources-black-hole-echoes-0502).

### 2020

- [Co-designing assistive technology around the globe — MIT Teaching + Learning Lab](https://tll.mit.edu/co-designing-assistive-technology-around-the-globe/).

### 2019

- [Co-designing assistive technologies in India — MIT News](https://news.mit.edu/2019/co-designing-assistive-technologies-india-0325).
- [Introduction to Disabilities and Assistive Technologies — MIT Open Learning](https://openlearning.mit.edu/mit-faculty/residential-digital-innovations/introduction-disabilities-and-assistive-technologies).

### 2018

- [Bringing humanistic education in technical subjects to the world — MIT News](https://news.mit.edu/2018/bringing-humanistic-education-in-technical-subjects-to-the-world-0628).
- [Spotlight: engineering staff Infinite Mile Awards — MIT News](https://news.mit.edu/2018/spotlight-engineering-staff-infinite-mile-awards-0530).
- [Q&A with Kyle Keane, lecturer and research scientist in MIT's Department of Materials Science and Engineering — MITili](https://mitili.mit.edu/news/qa-kyle-keane-lecturer-and-research-scientist-mits-department-materials-science-and).

### 2017

- [Learn to Build Your Own Videogame — Instructor Insights on MIT OpenCourseWare](https://ocw.mit.edu/courses/res-3-003-learn-to-build-your-own-videogame-with-the-unity-game-engine-and-microsoft-kinect-january-iap-2017/pages/instructor-insights/).
- [Collaborative Design and Creative Expression with Arduino — Instructor Insights on MIT OpenCourseWare](https://ocw.mit.edu/courses/res-3-002-collaborative-design-and-creative-expression-with-arduino-microcontrollers-january-iap-2017/pages/instructor-insights/).

### 2015

- [Materials science master class — MIT News](https://news.mit.edu/2015/materials-science-master-class-0707).

## Skills

### Programming languages

- **Fluent:** Mathematica / Wolfram Language; JavaScript, HTML, CSS.
- **Experienced:** Git; Bash; Windows PowerShell; Python; Arduino;
  Processing; Java; PHP.
- **Familiar:** Perl; Lua; BASIC; Fortran; R; C++.

### Software development

Large multi-contributor Python codebases for AI in interdisciplinary
research; an extension of Wolfram Mathematica for online classes with
curriculum-aligned assignments; a full-stack PHP-and-web LMS with
automated grading and analytics; web demonstrations of accessible
scientific graphics; APIs for ML, image processing, and text analysis;
the XML parser and natural-language generation that allow Apple Siri
to speak quantitative results from queries handled by Wolfram\|Alpha;
physics language parsers for Wolfram\|Alpha; the first generalised
step-by-step computational problem solver handling arbitrary units,
deployed in the
[Wolfram\|Alpha step-by-step solver](https://www.wolframalpha.com/examples/pro-features/step-by-step-solutions).

### Accessibility

Section 508 assessments of desktop and web applications; published
literature review of accessibility standards for dynamic educational
content; published report on best practices for verbal description of
interactive digital educational content; managed a small team of
accessibility representatives across departments; maintained VPATs and
provided technical solutions for blind users.

### Quantitative analysis

Open-source statistical forecasting models for urban university
campuses; programmed software features strengthening educational
software product offerings; data analysis template design for an
electron scattering experiment; quantum simulations (numerical and
analytical); matrix differential equations.

### Organisational and administrative

Multiple concurrent research grants written and managed; teams of
software developers managed for academic-research tools; planning,
organising, and execution of multiple workshops on technology and
computer programming for elementary children; statistics instructor
at a high-school summer camp; founder and organiser of a weekly
graduate-student colloquium; founder and host of a weekly free yoga
class for graduate students; initiator of a Hacker Night for
co-workers.

## Conferences and events attended

### Science and engineering

- Annual MRS Meeting: 2015, 2016, 2017
- March Meeting of APS: 2010, 2011, 2012, 2015
- AAAS Annual Meeting: 2013, 2014, 2016
- Joint Mathematics Meeting: 2014
- Coherence in Superconducting Qubits (Army Research Office): 2010
- Quantum Computation and Quantum Algorithms Program Review (IARPA): 2009
- DAMOP Meeting of the American Physical Society: 2007

### Software and education

- Scratch@MIT: 2014, 2016
- LearnLaunch: 2013, 2014, 2016
- Wolfram Technology Conference: 2012, 2013, 2015, 2016, 2017

### Accessibility and disability advocacy

- HCR@Work: 2023
- National Federation of the Blind: 2023
- CSUN Assistive Technology: 2018, 2019, 2020, 2022, 2023
- DIAGRAM Research Meeting: 2014, 2017
- ATIA Annual Meeting: 2014

### Hackathons mentored

- HackMIT: 2018, 2019, 2022
- HackNYU: 2018
- HackPrinceton: 2017, 2018
- ATHack: 2016, 2017, 2018
- Hacking Arts: 2016, 2017
